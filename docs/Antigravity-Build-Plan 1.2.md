# Alpha Vantage Explorer Build Plan v1.2 - Granular API Extension
...
2. Run `alphavantage_cli --help` → shows both bulk and granular subcommands
3. Run `alphavantage_cli overview --help` → shows symbol, output, live-api flags
4. Run `cargo test --package alphavantage_cli` → all tests pass
5. Test backward compatibility: `cargo run -- --symbols AAPL` → works as before

OUTPUT:
Report:
- ✅ Subcommand structure implemented
- ✅ 13 endpoint commands defined
- ✅ Parameter validation added
- ✅ Help documentation auto-generated by clap
...
2. Create a mock endpoint with 100 rows → verify all 100 rows in markdown
3. Test CSV endpoint (LISTING_STATUS) → verify raw CSV is saved, no markdown table
4. Check metadata notes → "(Showing 3 of 50 total rows)" appears correctly
5. Run `cargo clippy` → no warnings

OUTPUT:
Report:
- ✅ Full output mode implemented
- ✅ CSV passthrough working
- ✅ Table metadata enhanced
- ✅ Tests passing for both truncated and full modes
```
...
11.3 Implement output workflow (crates/cli/src/granular_executor.rs):
   ...
   - Branch on content-type:
     - If JSON:
       ...
       d. Print: "✓ Saved: {md_path} and {json_path}"
     - If CSV:
       a. Save raw CSV to `out/raw/{filename}.csv`
       b. Print: "✓ Saved: {csv_path} (CSV format, no markdown conversion)"

11.4 Error handling for granular mode (crates/cli/src/granular_executor.rs):
   - Catch rate limit errors → print user-friendly message with reset time
   - Catch network errors → print retry suggestion
   - Catch parse errors → save raw anyway, print error message
...
1. Test granular call (mock mode): `cargo run -- overview --symbol AAPL`
   → Check files exist: `out/overview_AAPL_{timestamp}.md` and `out/raw/overview_AAPL_{timestamp}.json`
2. Test CSV endpoint: `cargo run -- earnings-calendar --symbol MARKET`
   → Check CSV file exists, no markdown generated
3. Test rate limit: trigger rate limit → verify message shows reset time
4. Run all unit tests: `cargo test --package alphavantage_cli granular_executor`
5. Test invalid symbol: `cargo run -- overview --symbol INVALID123` → friendly error

OUTPUT:
Report:
- ✅ Endpoint routing complete
- ✅ Single-fetch method working
- ✅ JSON and CSV workflows functional
- ✅ Timestamped output files generated
...
12.4 Generate man pages (docs/man/ - NEW DIRECTORY):
   ...
   - Commands:
     - `man alphavantage_cli` → main help
     - `man alphavantage_cli-overview` → endpoint help
...
12.6 Add inline help (crates/cli/src/help.rs - NEW FILE):
   - Function: `print_endpoint_help(cmd: &str)` → rich help for each command
   - Use `clap`'s built-in `--help` generation
   - Enhance with examples and notes about output formats

VERIFICATION:
1. Read `README.md` → both modes clearly documented
2. Read `docs/manual/commands.md` → all 13 endpoints documented
3. Run `alphavantage_cli --help` → lists subcommands
4. Run `alphavantage_cli overview --help` → shows parameters and output format
5. Build man pages: `cargo build` → check `target/man/` directory
6. Review CHANGELOG.md → v0.2.0 entry complete

OUTPUT:
Report:
- ✅ README.md updated with dual-mode documentation
- ✅ Manual pages created for all endpoints
- ✅ Output format guide complete
...
13.1 Unit tests for granular executor (crates/cli/tests/granular_tests.rs - NEW FILE):
   - Test: `test_endpoint_routing()` → verify all 13 commands route correctly
   - Test: `test_parameter_validation()` → invalid quarter/year/horizon rejected
   - Test: `test_filename_generation()` → timestamps and naming correct
   - Test: `test_output_path_creation()` → directories created as needed
...
13.2 Integration tests for granular mode (crates/cli/tests/cli_granular_integration.rs - NEW FILE):
   - Test: `test_granular_json_workflow()` → mock JSON endpoint → verify MD + raw JSON
   -Test: `test_granular_csv_workflow()` → mock CSV endpoint → verify raw CSV only
   - Test: `test_granular_rate_limit()` → trigger rate limit → verify error message
   - Test: `test_full_output_retention()` → endpoint with 50 rows → verify all 50 in markdown
...
13.3 Backward compatibility tests (crates/cli/tests/bulk_mode_regression.rs - NEW FILE):
   - Test: `test_bulk_mode_unchanged()` → run existing `--symbols AAPL` → verify output matches v0.1.0
   - Test: `test_bulk_truncation()` → verify bulk mode still truncates to 3 rows
   - Compare output snapshots using `insta` crate

13.4 CSV handler tests (crates/client/tests/csv_handler_tests.rs - NEW FILE):
   - Test: `test_csv_detection()` → verify Content-Type header parsing
   - Test: `test_csv_save()` → verify CSV written correctly
   - Test: `test_csv_no_conversion()` → ensure no markdown generation for CSV

13.5 Parameter validation tests (crates/core/tests/param_validation_tests.rs - NEW FILE):
   - Test: `test_valid_quarters()` → Q1, Q2, Q3, Q4 accepted
   - Test: `test_invalid_quarters()` → Q5, INVALID rejected
   - Test: `test_year_bounds()` → 1900-2100 accepted, others rejected
   - Test: `test_horizon_values()` → valid horizons accepted

13.6 Run quality gates:
   - `cargo fmt --all -- --check` → MUST pass (fix any issues immediately)
   - `cargo clippy --all-targets --all-features -- -D warnings` → MUST pass
   - `cargo test --workspace` → all tests pass
   - `cargo build --release` → builds successfully

VERIFICATION:
1. Run `cargo test --workspace` → all tests pass (print test count)
2. Run `cargo clippy` → zero warnings
3. Run `cargo fmt --check` → no formatting issues
4. Run bulk mode test: `cargo run -- --symbols AAPL` → output matches v0.1.0
5. Run granular test: `cargo run -- overview --symbol AAPL` → files created correctly

OUTPUT:
Report:
- ✅ 25+ new tests added (granular + regression)
...
VERIFICATION:
1. Run CI locally: `.github/workflows/ci.yml` → all steps pass
2. Test pre-push hook: attempt push without fmt → blocked
3. Verify version consistency: `grep -r 'version = "0.2.0"' Cargo.toml`
4. Build release binary: `cargo build --release` → check `target/release/alphavantage_cli`
5. Run release binary: `target/release/alphavantage_cli --version` → shows "0.2.0"

OUTPUT:
Report:
- ✅ GitHub Actions enhanced with fmt/clippy gates
- ✅ Pre-push hook preventing CI failures
- ✅ Version numbers updated to 0.2.0
- ✅ Release checklist created
- ✅ Ready for v0.2.0 tag
```
...
15.2 Test CSV endpoint (EARNINGS_CALENDAR):
   ...
   - Verify:
     ...
     - Console output: "✓ Saved CSV to {path}"
   - Open CSV file → verify it's valid CSV format
...
15.4 Test bulk mode (regression):
   - Run: `cargo run --release -- --symbols AAPL --live-api --out-dir ./validation-bulk`
   - Verify:
     - All 13 endpoints attempted
     - Output matches v0.1.0 structure
     - Truncation to 3 rows still works
   - Compare with v0.1.0 output (manual spot-check)

15.5 Test rate limit handling:
...
OUTPUT:
Report:
- ✅ JSON endpoint validated (OVERVIEW)
- ✅ CSV endpoint validated (EARNINGS_CALENDAR)
- ✅ Optional parameters working (NEWS_SENTIMENT)
- ✅ Bulk mode backward compatible
...
- **Color output**: Use terminal colors for success (green ✓) and errors (red ✗)
...
4. ✅ Live API validation successful (all 3 test cases)
5. ✅ Backward compatibility verified (spot-check v0.1.0 output)
6. ✅ Documentation complete and reviewed
7. ✅ GitHub Actions CI passing
8. ✅ Pre-push hook installed and working

Release ready when:
1. ✅ All 15 phases complete
2. ✅ Version bumped to 0.2.0
3. ✅ CHANGELOG.md updated
4. ✅ Git tag `v0.2.0` created
